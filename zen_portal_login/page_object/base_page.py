...
class BasePage:
    ...
    def enter_text(self, locator, text , field_name):
        element = self.get_web_driver_wait().until(EC.visibility_of_element_located(locator))
        element.clear()
        element.send_keys(text)
        logger.info(f"Entered details on {field_name}")


    def click(self, locator, field_name):
        element = self.get_web_driver_wait().until(EC.element_to_be_clickable(locator))
        element.click()
